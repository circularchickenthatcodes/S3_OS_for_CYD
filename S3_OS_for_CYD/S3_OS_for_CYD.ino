...
static int l_getPressedKey(lua_State *L) {
  if (lastRawKey != 0) {
    // If it's a printable character, push the string/char
    if (lastKey >= 32 && lastKey <= 126) {
      char s[2] = {lastKey, 0};
      lua_pushstring(L, s);
    } else {
      // Otherwise push the raw HID hex code as an integer
      lua_pushinteger(L, lastRawKey);
    }
    // We don't reset lastRawKey here so multiple calls 
    // in one frame work, but you can if you want a "one-shot" press.
    return 1;
  }
  lua_pushnil(L);
  return 1;
...
void BLETaskCode(void * pvParameters) {
  ...
  while(true) {
    NimBLEScan* pScan = NimBLEDevice::getScan(); pScan->setActiveScan(true);
      ...
      if (device.getName() == targetDeviceName) {
        ...
        if (pClient->connect(&device)) {
          ...
          if (pSvc) {
            ...
            for (auto pChr : *chars) {
              if (pChr->getUUID() == NimBLEUUID((uint16_t)0x2A4D) && pChr->canNotify()) pChr->subscribe(true, notifyCallback);
            }
          }
        }
      }
...
void setup() {
  ...
  xTaskCreatePinnedToCore(BLETaskCode, "BLETask", 20000, NULL, 1, NULL, 0);
